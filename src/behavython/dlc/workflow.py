...
def _get_ffmpeg_bin_dir() -> str:
    from behavython.config.paths import FFMPEG_BIN_DIR
...
def _get_video_fps(video_path: str) -> float:
    ...
    result = subprocess.run(cmd, cwd=_get_ffmpeg_bin_dir(), capture_output=True, text=True, check=True)
...
def run_extract_frames(request: DLCFrameExtractionRequest, progress=None, log=None, warning=None):
    ...
    for index, video_path in enumerate(request.video_paths, start=1):
        ...
        subprocess.run(cmd, cwd=_get_ffmpeg_bin_dir(), capture_output=True, text=True, check=True)
...
def run_clear_unused_files(request: DLCClearUnusedFilesRequest, progress=None, log=None, warning=None):
    ...
    video_suffixes = {".mp4", ".avi", ".mov", ".mkv", ".wmv", ".flv", ".webm"}
